...
def EX2():
    ...
    for i in range(100):
        data = np.random.normal(loc=10, scale=1, size=10 * (i + 1))
        my_gaussian.fit(data)
        mean[i] = my_gaussian.get_mean()
    mean -= 10
    mean = list(mean)
...
def EX5():
    ...
    cov_matrix = np.array([[1, 0.2, 0, 0.5],
                           ...
                           [0, 0, 1, 0],
                           [0.5, 0, 0, 1]])
    samples = np.random.multivariate_normal(mu_matrix, cov_matrix, size=1000)
    ...
    for f1_value in f1_values:
        ...
        log_likehood.append(copy.deepcopy(log_likehood_row))
